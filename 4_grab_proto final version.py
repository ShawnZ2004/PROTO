...
#--TEST Protos 
            if False:
                if list(elements_new)!= sorted(list(elements_new)):
                    break
                
                ox_post_dec=zip(*sorted(zip(sorted_dec,elements_new)))
                if ox_post_dec != old_oxstates:
                    break
                ...
                if duplicate_positions != {}:
                    counted={}
                    for i in range(NARY):
                        if match_ox[i] in duplicate_positions.keys():
                            if match_ox[i] not in counted.keys():   
                                counted[match_ox[i]]=1
                            num*=(match_ox[i]-counted[match_ox[i]])
                            counted[match_ox[i]]=counted[match_ox[i]]+1
                        else:
                            num*=match_ox[i]
                else:
                    for i in range(NARY):
...
################################################################
#test ideas:
    #they basically checked anion exists and all atoms match between ? and ?
    #if original makefile has it, we should have it as well
    #
    #test proto:
    #check atoms are listed in alphabetical order
    #check order of oxstate after applying dec matcges the original oxidation state
    #script for calculating how many there should be based on manual (including formula for exception of repeat ox-state) and comparing with how many our product got
